...
class DAM:
    def __init__(self, babi_train_raw, babi_test_raw, word2vec,
                 word_vector_size, dim, mode, answer_module,
                 input_mask_mode, memory_hops, batch_size, l2,
                 normalize_attention, batch_norm, dropout, **kwargs):
        """
        Class of Dynamical Attentive Memory Net.
        :param babi_train_raw: training data.
        :param babi_test_raw: testing data.
        :param word2vec: word embedding.
        :param word_vector_size: word embedding size.
        :param dim: the dimension of hidden layer size.
        :param mode: train or test.
        :param answer_module: Feed-forward nets or recurrent nets.
        :param input_mask_mode: word by word, or sentence.
        :param memory_hops: memory hops.
        :param batch_size: size of mini-batch training.
        :param l2: hyper-parameter for normalization weights.
        :param normalize_attention: hyper-parameter for normalization
                attention model weights.
        :param batch_norm: batch normalization.
        :param dropout: the ratio of dropout.
        :param kwargs: hyper-parameter of model.
        :return: None.
        """
        ...
        print "# creating parameters for input module"
        self.W_inp_res_in, self.W_inp_res_hid, self.b_inp_res, \
        self.W_inp_upd_in, self.W_inp_upd_hid, self.b_inp_upd, \
        self.W_inp_hid_in, self.W_inp_hid_hid, self.b_inp_hid = \
            self.init_input_module()
        self.inp_c, self.q_q = self.init_input_module_context_question()

        print "# creating parameters for memory module"
        self.W_mem_res_in, self.W_mem_res_hid, self.b_mem_res, \
        self.W_mem_upd_in, self.W_mem_upd_hid, self.b_mem_upd, \
        self.W_mem_hid_in, self.W_mem_hid_hid, self.b_mem_hid, \
        self.W_b, self.W_1, self.W_2, self.b_1, \
        self.b_2 = self.init_memory_module()

        print "# building episodic memory module, fixed number of steps: " \
              "%d)" % self.memory_hops
        last_mem = self.building_episodic_memory_module()

        print "# creating parameters for answer module"
        self.W_a = util.normal_param(
            std=0.1, shape=(self.vocab_size, self.dim))
        if self.answer_module == 'feedforward':
            ...
            self.W_ans_hid_in, self.W_ans_hid_hid, self.b_ans_hid = \
                self.init_answer_module_recurrent_parameter()
            self.prediction = self.init_answer_module_recurrent_prediction(last_mem)
        else:
            raise Exception("invalid answer_module")
        self.prediction = self.prediction.dimshuffle(1, 0)

        print("# setting model parameters")
        self.params = self.set_model_parameter()

        print "# building loss layer and computing updates"

        self.loss_ce, self.loss_l2, self.loss = \
            self.building_loss_function()

        print "# setting weight update function"
    ...
    def building_loss_function(self):
        _loss_ce = T.nnet.categorical_crossentropy(
            self.prediction, self.answer_var).mean()
        if self.l2 > 0:
            _loss_l2 = self.l2 * util.l2_reg(self.params)
        else:
            _loss_l2 = 0
        return _loss_ce, _loss_l2, _loss_ce + _loss_l2

    def init_answer_module_recurrent_parameter(self):
        _W_ans_res_in = util.normal_param(
            std=0.1, shape=(self.dim, self.dim + self.vocab_size))
        _W_ans_res_hid = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _b_ans_res = util.constant_param(
            value=0.0, shape=(self.dim,))

        _W_ans_upd_in = util.normal_param(
            std=0.1, shape=(self.dim, self.dim + self.vocab_size))
        _W_ans_upd_hid = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _b_ans_upd = util.constant_param(value=0.0, shape=(self.dim,))

        _W_ans_hid_in = util.normal_param(
            std=0.1, shape=(self.dim, self.dim + self.vocab_size))
        _W_ans_hid_hid = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _b_ans_hid = util.constant_param(value=0.0, shape=(self.dim,))

        return _W_ans_res_in, _W_ans_res_hid, _b_ans_res, \
               _W_ans_upd_in, _W_ans_upd_hid, _b_ans_upd, \
               _W_ans_hid_in, _W_ans_hid_hid, _b_ans_hid

    def answer_step(self, prev_a, prev_y):
        a = self.GRU_update(
            prev_a, T.concatenate([prev_y, self.q_q]),
            self.W_ans_res_in, self.W_ans_res_hid, self.b_ans_res,
            self.W_ans_upd_in, self.W_ans_upd_hid, self.b_ans_upd,
            self.W_ans_hid_in, self.W_ans_hid_hid, self.b_ans_hid)
    ...
    def init_answer_module_recurrent_prediction(self, last_mem):
        ...
        results, updates = theano.scan(
            ...
            n_steps=1)
        _prediction = results[1][-1]
        return _prediction

    def set_model_parameter(self):
        _params = [self.W_inp_res_in, self.W_inp_res_hid, self.b_inp_res,
                   self.W_inp_upd_in, self.W_inp_upd_hid, self.b_inp_upd,
                   self.W_inp_hid_in, self.W_inp_hid_hid, self.b_inp_hid,
                   self.W_mem_res_in, self.W_mem_res_hid, self.b_mem_res,
                   self.W_mem_upd_in, self.W_mem_upd_hid, self.b_mem_upd,
    ...
    def init_memory_module(self):
        _W_mem_res_in = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _W_mem_res_hid = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _b_mem_res = util.constant_param(value=0.0, shape=(self.dim,))

        _W_mem_upd_in = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _W_mem_upd_hid = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _b_mem_upd = util.constant_param(value=0.0, shape=(self.dim,))

        _W_mem_hid_in = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _W_mem_hid_hid = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _b_mem_hid = util.constant_param(value=0.0, shape=(self.dim,))

        _W_b = util.normal_param(std=0.1, shape=(self.dim, self.dim))

        _W_1 = util.normal_param(
            std=0.1, shape=(self.dim, 7 * self.dim + 0))
        _W_2 = util.normal_param(std=0.1, shape=(1, self.dim))

        _b_1 = util.constant_param(value=0.0, shape=(self.dim,))
        _b_2 = util.constant_param(value=0.0, shape=(1,))

        return _W_mem_res_in, _W_mem_res_hid, _b_mem_res, \
               _W_mem_upd_in, _W_mem_upd_hid, _b_mem_upd, \
               _W_mem_hid_in, _W_mem_hid_hid, _b_mem_hid, \
               _W_b, _W_1, _W_2, _b_1, _b_2

    def building_episodic_memory_module(self):
        memory = [self.q_q.copy()]
        for iter in range(1, self.memory_hops + 1):
            current_episode = self.new_episode(memory[iter - 1])
            memory.append(self.GRU_update(
                memory[iter - 1], current_episode,
                self.W_mem_res_in, self.W_mem_res_hid, self.b_mem_res,
                self.W_mem_upd_in, self.W_mem_upd_hid, self.b_mem_upd,
                self.W_mem_hid_in, self.W_mem_hid_hid, self.b_mem_hid))
        last_mem_raw = memory[-1].dimshuffle((1, 0))
        net = layers.InputLayer(
            shape=(self.batch_size, self.dim), input_var=last_mem_raw)
        if self.batch_norm:
    ...
    def init_input_module(self):
        _W_inp_res_in = util.normal_param(
            std=0.1, shape=(self.dim, self.word_vector_size))
        _W_inp_res_hid = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _b_inp_res = util.constant_param(value=0.0, shape=(self.dim,))

        _W_inp_upd_in = util.normal_param(
            std=0.1, shape=(self.dim, self.word_vector_size))
        _W_inp_upd_hid = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _b_inp_upd = util.constant_param(value=0.0, shape=(self.dim,))

        _W_inp_hid_in = util.normal_param(
            std=0.1, shape=(self.dim, self.word_vector_size))
        _W_inp_hid_hid = util.normal_param(
            std=0.1, shape=(self.dim, self.dim))
        _b_inp_hid = util.constant_param(value=0.0, shape=(self.dim,))
    ...
    def init_input_module_context_question(self):
        ...
        for batch_index in range(self.batch_size):
            ...
            inp_c_list.append(
                T.concatenate([
                    taken,
                    T.zeros(
                        (self.input_mask_var.shape[1] - taken.shape[0],
                         self.dim
                         ),
                        floatX
                    )
        ...
        _inp_c = T.stack(inp_c_list).dimshuffle(1, 2, 0)
    ...
    def GRU_update(self, h, x,
                   W_res_in, W_res_hid, b_res,
        ...
        :return:
        """
        z = T.nnet.sigmoid(T.dot(W_upd_in, x) +
                           T.dot(W_upd_hid, h) +
                           b_upd.dimshuffle(0, 'x'))
        r = T.nnet.sigmoid(T.dot(W_res_in, x) +
                           T.dot(W_res_hid, h) +
                           b_res.dimshuffle(0, 'x'))
        _h = T.tanh(T.dot(W_hid_in, x) +
                    r * T.dot(W_hid_hid, h) +
                    b_hid.dimshuffle(0, 'x'))
        return z * h + (1 - z) * _h
        ...
        with open(file_name, 'w') as save_file:
            pickle.dump(
                obj={
                    ...
                    'epoch': epoch,
                    'gradient_value': (
                        kwargs['gradient_value'] if 'gradient_value' in kwargs
                        else 0)
                },
                file=save_file,
                protocol=-1
    ...
    def _process_input(self, data_raw):
        ...
        for x in data_raw:
            ...
            answers.append(word.process_word(word=x["A"],
                                             word2vec=self.word2vec,
                                             vocab=self.vocab,
                                             ivocab=self.ivocab,
                                             word_vector_size=self.word_vector_size,
    ...
    def shuffle_train_set(self):
        ...
        self.train_answer, \
        self.train_fact_count, \
        self.train_input_mask = zip(*combined)
    ...
    def step(self, batch_index, mode):
        ...
        param_norm = np.max([util.get_norm(x.get_value()) for x in self.params])
